...
function buildField(
  ...
  const isPrimaryKey =
    identityFields.includes(field.name);
...
function buildEntity(
  ...
  const entityForeignKeyFields =
    foreignKeyFields.get(entity.name) ??
    new Set<string>();
...
function endpointEntityName(
  endpoint: string,
): string {
  return endpoint.split('.')[0];
}

function cardinalityForRelationship(
  type: string,
  side: 'source' | 'target',
): string {
  switch (type) {
  ...
  return specification.relationships.map(
    relationship => {
      ...
      const matchingForeignKeys =
        specification.foreignKeys.filter(
          foreignKey =>
            (
...
export function adaptSpecification(
  ...
  const foreignKeyFields =
    buildForeignKeyFieldMap(
      specification.foreignKeys,
    );
