...
const DEFAULT_LAYOUT: LayoutOptions = {
  /*
   * Spacing is derived from the entity dimensions rather than being
   * independent magic numbers.
   *
   * This keeps the layout proportional if the entity card dimensions
   * change later.
   */
...
function rectangleOverlaps(
  ...
  const candidateRight = position.x + MODEL_NODE_WIDTH + gapX;
  const candidateTop = position.y - gapY;
  const candidateBottom = position.y + MODEL_NODE_HEIGHT + gapY;
  ...
  for (const entity of entities) {
    ...
    const existingRight = existing.x + MODEL_NODE_WIDTH;
    const existingTop = existing.y;
    const existingBottom = existing.y + MODEL_NODE_HEIGHT;
...
function candidatePositions(
   ...
   * Primary preference:
   *
   * Keep related entities beside each other horizontally.
   *
   * This prevents the layout from naturally degenerating into
   * long vertical chains such as:
   *
   *     A
   *     |
   *     B
   ...
   *
   * Diagonal placement gives the graph room to spread when a
   * horizontal position is already occupied.
   */
  for (const neighbour of neighbours) {
    candidates.push(
      {
  ...
  /*
   * Vertical placement is deliberately last.
   *
   * It remains available when the surrounding graph makes it
   * necessary, but it is no longer the normal placement strategy.
   */
  for (const neighbour of neighbours) {
    candidates.push(
      {
...
function scoreCandidate(
  ...
  /*
   * Distance keeps connected entities close.
   *
   * A vertical-alignment penalty prevents the algorithm from
   * repeatedly stacking entities directly above/below each other.
   */
...
function findBestPosition(
  ...
  const neighbours = [...(graph.get(entity.name) ?? [])]
  ...
  if (validCandidates.length === 0) {
    /*
     * If all preferred positions are occupied, progressively search
     * around the connected neighbourhood.
     */
    ...
    const validFallbacks = fallbackCandidates.filter(
      candidate =>
        candidate.x >= options.canvasPadding &&
        candidate.y >= options.canvasPadding &&
        !rectangleOverlaps(
...
export function layoutModel(
  ...
  /*
   * Start with the most connected entity.
   *
   * This tends to place the core of the model near the centre of
   * the resulting layout and lets related entities grow outward.
   */
  ...
  while (remaining.size > 0) {
    /*
     * Prefer the unplaced entity with the largest number of
     * already-positioned neighbours.
     */
